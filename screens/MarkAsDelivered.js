...
import { View, Image, Text, AppRegistry, PermissionsAndroid, ImageBackground, Alert } from 'react-native';
...
export default class Delivered extends Component {
    constructor(props) {
        ...
        let letterStatus =''; // eslint-disable-line
    ...
    deliveredButton() {
        if (!(this.state.dataId === null)) {
                ...
                fetch('http://192.168.100.146:4545/users', {
                    ...
                    if (jsonData['changedRows'] == 1) {
                        if (this.type === 'NormalPost')
                            Alert.alert('Incremented the Normal Post delivered count')
                        else if (this.type === 'Parcel')
                            Alert.alert(this.type + ' marked as ' + this.letterStatus)
                    }
                    else {
                        Alert.alert('Nothing Changed in Database. Scan again')
                    }
                    this.letterStatus = '';
                }).done();
                    ...
                    fetch('http://192.168.100.146:4545/users', {
                        ...
                        if (jsonData['changedRows'] == 1) {
                            if (this.letterStatus === 'delivered' || this.letterStatus === 'sent-back')
                                ...
                                Alert.alert('Something Wrong. May be letter is not in an accepted state to deliver')
                            }
                        }
                        else {
                            Alert.alert('Nothing Changed in Database. Scan again')
            ...
            Alert.alert('Id is not Detected. Please scan again or Enter the ID and submit')
        }
    }
        ...
        if (!(this.state.dataId === null)) {
            if (this.type === 'NormalPost' || this.type === 'Parcel') {
                ...
                fetch('http://192.168.100.146:4545/users', {
                    ...
                    if (jsonData['changedRows'] == 1) {
                        if (this.type === 'NormalPost')
                            Alert.alert('Incremented the Normal Post failed delivered count')
                        else if (this.type === 'Parcel')
                            Alert.alert(this.type + ' marked as ' + this.letterStatus)
                    }
                    else {
                    ...
                    fetch('http://192.168.100.146:4545/users', {
                        ...
                        if (jsonData['changedRows'] == 1) {
                            if (this.letterStatus == 'receiver-unavailable' || this.letterStatus == 'sender-unavailable')
                                ...
                                Alert.alert('Something Wrong. May be letter is not in an accepetd state to mark undelivered')
                            }
                        }
                        else {
                            Alert.alert('Nothing Changed in Database. Scan again')
